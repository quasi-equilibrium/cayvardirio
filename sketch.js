...
const THEME = {
  ...
  rain: [200,200,255,90],
  compass: [255,220,90],
  l2ArcBlue: [80,160,255]  // L2 mavi yay rengi
};
...
function baseResetCommon(){
  ...
  // Kamera başlangıçta oyuncuya sabitlenir + hemen güncelle
    ...
    if (!isL2){
      ...
      if (e.type==="mirror"){
        ...
        if (e.cd<=0){
          e.cd = 1.4;
          for (let i=-1;i<=1;i++){
            const ang = Math.atan2(py - e.y, px - e.x) + i*0.15;
...
function drawEnemiesAndBullets(){
  for (let e of enemies){
    if (!isL2){
      ...
      if (e.type==="mirror"){
        imageMode(CENTER);
        if (l2_mirror&&l2_mirror.width) image(l2_mirror,e.x,e.y,60,60);
        else { fill(180); rect(e.x-24,e.y-24,48,48,6); fill(30); textAlign(CENTER,CENTER); text("MIR", e.x, e.y); }
      }
      else if (e.type==="swing"){
        imageMode(CENTER);
        if (l2_swing&&l2_swing.width) image(l2_swing,e.x,e.y,46*6,46*6);
  ...
  for (let b of bullets){
    if (!b.wave){
      ...
      if (b.arc){
        noFill();
        const col = b.arcColor || (isL2? THEME.l2ArcBlue : [255,255,255]);
...
function setup(){
  // 1) Retina/çeyrek görünüm riskini azalt
  pixelDensity(1);

  // 2) Canvas'ı oluştur ve CSS boyutunu attr ile birebir eşitle (KRİTİK)
